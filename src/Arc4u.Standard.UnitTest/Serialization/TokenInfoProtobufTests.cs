...
{
    public class TokenInfoProtobufTests : BaseContainerFixture<TokenInfoProtobufTests, TokenProtobufSerializerFixture>
    {
        public TokenInfoProtobufTests(TokenProtobufSerializerFixture containerFixture) : base(containerFixture)
        {
        }


        [Fact]
        public void Test_TokenInfo_Protobuf_GetTokenInfo()
        {
            using (var container = Fixture.CreateScope())
            {
                LogStartBanner();

                var logger = container.Resolve<ILogger<TokenInfoProtobufTests>>();
