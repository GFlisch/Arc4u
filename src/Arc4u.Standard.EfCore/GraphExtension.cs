...
namespace Arc4u.EfCore
{
    public static class GraphExtension
    {
        ...
        {
            return graph.Includes.Aggregate<string, IQueryable<T>>(query, (queryable, i) => queryable.BuildInclude(i));
        }
        ...
        private static IQueryable<T> BuildInclude<T>(this IQueryable<T> queryable, string propertyPath) where T : class
        {
            ...
            object thenIncludeObject = BuildInternalInclude(queryable, properties[0], out var propertyType);
        ...
        private static object BuildInternalInclude<T>(IQueryable<T> queryable, string property, out Type propertyType) where T : class
        {
            ...
            return includeMethodInfo.MakeGenericMethod(new[] { typeof(T), propertyType }).Invoke(queryable, new object[] { queryable, expr });
        }
