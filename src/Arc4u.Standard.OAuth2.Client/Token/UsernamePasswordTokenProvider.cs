...
{
    [System.Composition.Export(UsernamePasswordTokenProvider.ProviderName, typeof(ITokenProvider))]
    public class UsernamePasswordTokenProvider : ITokenProvider
    {
        ...
        public async Task<TokenInfo> GetTokenAsync(IKeyValueSettings settings, object platformParameters)
        {
            ...
            if (String.IsNullOrWhiteSpace(upn) || String.IsNullOrWhiteSpace(pwd))
            {
                ...
                var hasCredential = await usernamePasswordProvider.GetCredentials(upn, CheckCredentialsAsync);
            ...
            try
            {
                ...
                tokenInfo = await CreateBasicTokenInfoAsync(settings, new CredentialsResult(true, upn, pwd));
        ...
        public async Task<bool> CheckCredentialsAsync(string upn, string password)
        {
            try
            {
                ...
                var tokenInfo = await CreateBasicTokenInfoAsync(Settings, new CredentialsResult(true, upn, password));
