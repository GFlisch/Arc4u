...
{
    /// <summary>
    /// The Rijndael class is used to encode and decode a string with the RijndaelManaged API.
    /// </summary>
    public class Rijndael
    {
        ...
        public static string DecodeCypherString(string cypherText, byte[] rgbKey, byte[] rgbIV)
        {
            ...
            using (var decryptor = myRijndael.CreateDecryptor(rgbKey, rgbIV))
        ...
        {
            using (var myRijndael = new RijndaelManaged())
            using (var encryptor = myRijndael.CreateEncryptor(rgbKey, rgbIV))
